...
var (
	SignKey   *rsa.PrivateKey //	llave privada: con esto firmo
	VerifyKey *rsa.PublicKey  //	lave publica: con esto puedo entregar a cualquier cliente
	once      sync.Once
)
...
func generateJWT(u Model) (string, error) {
	...
	c := Claim{
		...
		StandardClaims: jwt.StandardClaims{
			...
			ExpiresAt: time.Now().Add(time.Hour * 24 * 1).Unix(), //	Tiempo: un dia
			Issuer:    "Cursos EDteam",                           //	Quien lo genero
		},
	}

	t := jwt.NewWithClaims(jwt.SigningMethodRS256, c) //	se firma con el RS256
